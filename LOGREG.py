...
def classify_lr(x_train, x_test, y_train, y_test):
    ...
    roc_score = metrics.roc_auc_score(y_test, predicted4, average='macro')
    print("roc_score of LogisticREG IS ->")
    print(metrics.roc_auc_score(y_test, predicted4, average='micro'))
    print('---------------------------------------------- ')
    # plot_confusion_matrix(classify4, x_test, y_test)
    # plt.title('Confusion Matrix of LogisticREG')
    # plt.show()
    val = ("LogisticREG", str(lr), str(precision), str(recall), str(f1_score), str(roc_score))
